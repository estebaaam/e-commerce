...
def create_cart(cart: cartSchema.Cart, db: Session = Depends(get_db)):
  return crudCart.create_cart(db=db, cart=cart)
  

@router.get("/cart/{id_usuario}", response_model=list[cartSchema.Cart])
def read_cart(id_usuario: int, db: Session = Depends(get_db)):
...
def update_cart(id_usuario: int, id_producto: int, cart: cartSchema.Cart, db: Session = Depends(get_db)):
...
def delete_cart(id_usuario: int, id_producto: int, db: Session = Depends(get_db)):
